...
**Action:** Always add an `sr-only` span containing a clarifying prefix (like "Yes: " or "No: ") to the label when the component's status is conveyed purely through color and icons, ensuring the meaning is fully communicated to assistive technologies.
>>>>>>> bff624a (feat: Add `sr-only` accessibility prefix to TrueFalseChip)
